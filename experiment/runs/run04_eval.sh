...
RUN_DIR="${RUN_ROOT}/eval/${EVAL_NAME}"
...
PYTHONPATH="${STAGE_ROOT}/python:${REPO_ROOT}/.deps:${LAB_ROOT}" "${TRAIN_PYTHON}" -u experiment/scripts/main_ppo_stage04.py \
  ...
  data.train_batch_size=64 data.val_batch_size=64 \
  ...
  actor_rollout_ref.actor.ppo_mini_batch_size=64 actor_rollout_ref.actor.ppo_micro_batch_size=8 \
  trainer.logger="['console']" +trainer.val_only=true +trainer.val_before_train=true \
  +trainer.trajectory_output="${RUN_DIR}/trajectories.jsonl" \
  trainer.n_gpus_per_node=8 trainer.nnodes=1 trainer.total_epochs=1 trainer.total_training_steps=1 \
  trainer.default_hdfs_dir=null trainer.default_local_dir="${RUN_DIR}/unused" \
  max_turns=2 do_search=true retriever.url=http://127.0.0.1:8014/retrieve retriever.topk=3 \
  2>&1 | tee "${RUN_DIR}/eval.log"

test "$(wc -l <"${RUN_DIR}/trajectories.jsonl")" -eq 128
grep -q "Initial validation metrics" "${RUN_DIR}/eval.log"
